...
#
# Multi-stage image for chapter-publishing-mcp-server.
# Prefer linux/arm64:
#   docker buildx build --platform linux/arm64 -t chapter-publishing-mcp-server:dev .
#   docker run --rm --platform linux/arm64 \
#     -e SOPS_AGE_KEY="$(cat ~/.config/sops/age/keys.txt)" chapter-publishing-mcp-server:dev
#
...
RUN --mount=type=cache,target=/usr/local/cargo/registry,id=cargo-registry,sharing=locked \
    ...
    --mount=type=cache,target=/src/target,id=chapter-publishing-mcp-server-target-${TARGETARCH},sharing=locked \
    cargo build --release --locked --bin chapter-publishing-mcp-server \
    && strip "target/release/chapter-publishing-mcp-server" \
    && cp "target/release/chapter-publishing-mcp-server" "/usr/local/bin/chapter-publishing-mcp-server"
...
COPY --from=build "/usr/local/bin/chapter-publishing-mcp-server" "/usr/local/bin/chapter-publishing-mcp-server"
...
RUN --mount=type=bind,source=.,target=/src,ro \
    ...
    && chown -R 65532:65532 /app /usr/local/bin/chapter-publishing-mcp-server
ENV SOPS_SECRETS_FILE=/app/secrets/app.env \
    OTEL_SERVICE_NAME=chapter-publishing-mcp-server \
    OTEL_EXPORTER_OTLP_ENDPOINT=http://dd-otel-collector.observability.svc.cluster.local:4317 \
    RUST_LOG=info
USER 65532:65532
EXPOSE 8080
ENTRYPOINT ["/usr/local/bin/sops-entrypoint.sh", "/usr/local/bin/chapter-publishing-mcp-server"]
